...
import { 
  ...
  RotateCcw, ArrowLeft, CheckCheck, Key, LogOut, Scale, Search
} from 'lucide-react';
...
import { AdminImageCheckDashboard } from './image-check/AdminImageCheckDashboard';
import { 
  auth, 
  db, 
  signInAnonymously, 
      ...
      {isConnecting ? (
        ...
        <div className="bg-slate-50 dark:bg-slate-950 p-6 rounded-2xl border border-blue-100 dark:border-blue-900/40 text-center space-y-4 py-8 animate-pulse text-xs">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[11px] text-gray-400">Synchronisation en temps réel avec la base de données...</p>
          </div>
        </div>
      ) : (
        <>
              ...
              {recoveryStep === 'request' ? (
                ...
                <form onSubmit={handlePerformPasswordReset} className="space-y-3.5">
                  ...
                  >
                    ...
                    <span>{isRecovering ? 'Enregistrement dans la base de données...' : 'Enregistrer le nouveau mot de passe'}</span>
                  </button>
                </form>
              )}
...
export function AdminDashboard({ 
  ...
  // Tabs: 'users' | 'moderation' | 'registry' | 'courses-creation' | 'communiques' | 'publicites' | 'analytics' | 'unes' | 'newsletters' | 'legal-pages' | 'videos' | 'image-check'
  const [adminActiveTab, setAdminActiveTab] = useState<'users' | 'moderation' | 'registry' | 'courses-creation' | 'communiques' | 'publicites' | 'analytics' | 'unes' | 'newsletters' | 'legal-pages' | 'videos' | 'image-check'>('users');
  ...
  const [adPlacement, setAdPlacement] = useState<'header' | 'sidebar' | 'footer' | 'in_feed' | 'popup' | 'above_rumors'>('sidebar');
  const [adPlacements, setAdPlacements] = useState<('header' | 'sidebar' | 'footer' | 'in_feed' | 'popup' | 'above_rumors')[]>(['sidebar']);
    ...
    triggerConfirm({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          triggerToast('Communiqué sauvegardé avec succès dans la base de données et diffusé !', 'success');
        } catch (err) {
  ...
  const handleAddAdvertisement = async (e: React.FormEvent) => {
    ...
    triggerConfirm({
      ...
      message: isEditing 
        ? `Voulez-vous enregistrer les modifications apportées à la publicité "${adTitle}" dans la base de données ?` 
      ...
      onConfirm: async () => {
        try {
          ...
          triggerToast(isEditing ? 'Publicité mise à jour dans la base de données avec succès !' : 'Publicité enregistrée dans la base de données et diffusée en temps réel !', 'success');
        } catch (err) {
    ...
    triggerConfirm({
      ...
      onConfirm: async () => {
        try {
          ...
          triggerToast("Erreur lors de la suppression de l'utilisateur sur la base de données.", "error");
        }
      }
    });
  };
  ...
  const handleDeleteDemoAccounts = () => {
    triggerConfirm({
      ...
      message: "Voulez-vous vraiment supprimer définitivement toutes les données de démonstration (comptes démo, rumeurs démo, et candidatures de médias démo) de la base de données ? Cette action est irréversible.",
      isDestructive: true,
      onConfirm: async () => {
  ...
  return (
    <div id="admin-workspace-master" className="space-y-8 animate-fade-in py-4 max-w-5xl mx-auto">
      ...
      <div className="flex flex-wrap border-b border-gray-150 dark:border-slate-800 gap-2">
        ...
        {isTabPermitted('image-check') && (
          <button
            onClick={() => setAdminActiveTab('image-check')}
            className={`pb-3 text-xs uppercase tracking-wider font-extrabold transition-all cursor-pointer px-1 flex items-center gap-1.5 border-b-2 ${
              adminActiveTab === 'image-check' 
                ? 'border-rose-600 text-rose-600 dark:text-rose-400' 
                : 'border-transparent text-gray-400 hover:text-gray-900 dark:hover:text-white'
            }`}
          >
            ...
            <div className="bg-gradient-to-r from-slate-900 via-rose-950/40 to-slate-900 border border-slate-800 p-5 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-4 text-white shadow-sm">
              <div className="space-y-1 text-center md:text-left">
                <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-rose-500/20 text-rose-300 text-[9px] font-black uppercase tracking-wider border border-rose-500/30 font-mono">
                  ...
                  <span>Base de Données • Référentiel National 2026</span>
                </div>
                ...
                <p className="text-xs text-slate-300 font-medium max-w-xl">
                  Vous disposez de tous les privilèges administratifs : création, modification des cours et exercices, suppression et synchronisation globale vers la base de données.
                </p>
              </div>
