...
export default function ContactSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Dispatch 3: Supabase backup persistence
    const sendDatabase = async () => {
      try {
        await supabaseDb.addDoc('contact_messages', payload);
      } catch (err) {
        console.warn('[Contact] Database persistence dispatch:', err);
      }
    };

    // Execute dispatches asynchronously
    Promise.allSettled([sendServerApi(), sendFormSubmit(), sendDatabase()]).then(() => {
