...
import { 
  ...
  Image as ImageIcon, Link2, Sparkles, CheckCheck
} from 'lucide-react';
...
import { ImageCheck } from './image-check/ImageCheck';
...
export default function VerificateurHaac({ 
  ...
  const [verificationMode, setVerificationMode] = useState<'url' | 'image'>(initialMode);

  useEffect(() => {
    if (initialMode) {
      setVerificationMode(initialMode);
    }
  }, [initialMode]);
...
Généré par le service indépendant de vérification d'informations basé sur les données de la HAAC.`;
  ...
  return (
    <div id="haac-verifier-main" className="space-y-6 max-w-4xl mx-auto">
      ...
      {/* Centre de Vérification ACTUHUB Header & Mode Selector */}
      <div id="verification-hub-header" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 rounded-3xl p-6 md:p-8 border border-slate-200 dark:border-slate-800 shadow-sm space-y-5 text-left">
        <div className="flex flex-col md:flex-row gap-5 items-start md:items-center justify-between">
          <div className="flex items-start md:items-center gap-4">
            <div className="bg-gradient-to-br from-blue-600 to-indigo-700 text-white p-4 rounded-2xl flex items-center justify-center shrink-0 shadow-lg shadow-blue-500/20">
              <ShieldCheck className="w-8 h-8" />
            </div>
            <div>
              <h1 className="text-xl md:text-2xl font-black tracking-tight text-slate-900 dark:text-white">
                Centre de Vérification ACTUHUB
              </h1>
            </div>
          </div>
        </div>

        {/* The Two Choice Cards: URL vs Image */}
        <div className="pt-2">
          <p className="text-[11px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-3 font-mono">
            Sélectionnez votre type de vérification :
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {/* Option 1: URL Link Verification */}
            <button
              id="choose-verification-url"
              type="button"
              onClick={() => setVerificationMode('url')}
              className={`p-5 rounded-2xl border-2 text-left transition-all cursor-pointer relative flex flex-col justify-between gap-3 ${
                verificationMode === 'url'
                  ? 'bg-blue-50/80 dark:bg-blue-950/40 border-blue-600 text-slate-900 dark:text-white shadow-md ring-2 ring-blue-500/20'
                  : 'bg-slate-50/60 dark:bg-slate-800/40 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:border-slate-700 hover:bg-slate-100/60'
              }`}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-[10px] font-black uppercase tracking-wider text-blue-600 dark:text-blue-400 font-mono block">Vérification Web</span>
                    <h2 className="text-sm font-black tracking-tight">Par lien URL</h2>
                  </div>
                </div>
                {verificationMode === 'url' && (
              ...
              <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed font-medium">
                Vérifier si un article ou nom de domaine (.bj) est agréé au registre officiel des 143 médias de la HAAC, son niveau de chiffrement SSL et son ton éditorial.
              </p>
              <div className="flex items-center gap-1.5 flex-wrap text-[10px] font-bold">
                <span className="bg-white dark:bg-slate-900 px-2 py-0.5 rounded-md border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300">Registre HAAC</span>
                <span className="bg-white dark:bg-slate-900 px-2 py-0.5 rounded-md border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300">Certificat SSL</span>
                <span className="bg-white dark:bg-slate-900 px-2 py-0.5 rounded-md border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300">Extension .BJ</span>
              </div>
            </button>

            {/* Option 2: Image Verification (Image Check) */}
            <button
              id="choose-verification-image"
              type="button"
              onClick={() => setVerificationMode('image')}
              className={`p-5 rounded-2xl border-2 text-left transition-all cursor-pointer relative flex flex-col justify-between gap-3 ${
                verificationMode === 'image'
                  ? 'bg-indigo-50/80 dark:bg-indigo-950/40 border-indigo-600 text-slate-900 dark:text-white shadow-md ring-2 ring-indigo-500/20'
                  : 'bg-slate-50/60 dark:bg-slate-800/40 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:border-slate-700 hover:bg-slate-100/60'
              }`}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] font-black uppercase tracking-wider text-indigo-600 dark:text-indigo-400 font-mono block">Forensique & IA</span>
                      <span className="bg-gradient-to-r from-indigo-500 to-purple-500 text-white text-[9px] font-black uppercase px-1.5 py-0.2 rounded font-mono shadow-xs">En expérimentation</span>
                    </div>
                    <h2 className="text-sm font-black tracking-tight">Vérification d'image</h2>
                  </div>
                </div>
                {verificationMode === 'image' && (
                  <span className="w-6 h-6 rounded-full bg-indigo-600 text-white flex items-center justify-center shrink-0 shadow-sm">
                    <Check className="w-3.5 h-3.5" />
                  </span>
                )}
              </div>
              <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed font-medium">
                Détection d'images IA (Midjourney, DALL-E, Flux), altérations forensiques (ELA), métadonnées EXIF, géolocalisation GPS, deepfake et signature C2PA.
              </p>
              <div className="flex items-center gap-1.5 flex-wrap text-[10px] font-bold">
                <span className="bg-white dark:bg-slate-900 px-2 py-0.5 rounded-md border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300">Détection IA</span>
                <span className="bg-white dark:bg-slate-900 px-2 py-0.5 rounded-md border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300">Forensique ELA</span>
                <span className="bg-white dark:bg-slate-900 px-2 py-0.5 rounded-md border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300">EXIF & GPS</span>
                <span className="bg-white dark:bg-slate-900 px-2 py-0.5 rounded-md border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300">Rapport PDF</span>
              </div>
            </button>
          </div>
        </div>
      ...
      {/* Dynamic View rendering depending on chosen verification mode */}
      {verificationMode === 'image' ? (
        <div id="image-verification-container" className="space-y-6 animate-fade-in duration-200">
          <ImageCheck currentUser={currentUser} onLoginClick={onLoginClick} />
        </div>
      ) : (
        <div id="url-verification-container" className="space-y-6 animate-fade-in duration-200">
