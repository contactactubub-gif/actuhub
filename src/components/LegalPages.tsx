...
export function LegalPageEditorModal({
  ...
  const triggerSave = () => {
    setConfirmDialog({
      ...
      message: `Les modifications apportées à "${formData.title}" seront enregistrées dans la Base de données et publiées immédiatement pour tous les utilisateurs.`,
      details: (
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          triggerToast(`Page "${formData.title}" enregistrée et synchronisée dans la Base de données !`, 'success');
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400 font-mono">
                Modifications instantanées synchronisées avec la Base de données
              </p>
            </div>
          </div>
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-blue-950 to-slate-950 text-white p-8 md:p-10 shadow-xl border border-slate-800">
        <div className="space-y-3">
          ...
          <div className="pt-2 text-[10px] text-slate-400 font-mono flex items-center gap-2">
            ...
            {pageData.updatedAt && (
              <span className="text-emerald-400 font-semibold">• Synchronisé Base de données</span>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Modifiez en temps réel la Politique de Confidentialité et les Conditions Générales d'Utilisation. Stockage chiffré dans la Base de données.
          </p>
        </div>
      </div>
