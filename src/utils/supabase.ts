...
try {
  ...
  try {
    firebaseDbInstance = initializeFirestore(firebaseApp, {
  ...
  console.warn('[Firebase] Fallback to primary Supabase client:', initErr);
}

export const firebaseDb = firebaseDbInstance;

export function isFirebaseCollection(_table: string): boolean {
  // Return false: Supabase is the primary, resilient database for ActuHub Benin.
  // Routing all collections directly to Supabase ensures instantaneous loading,
  // real-time synchronization, and eliminates Firestore WebChannel connection unavailable errors.
  return false;
}
...
export const supabaseDb = {
  ...
  normalizeRecord<T = any>(table: string, item: any): T {
    ...
    if (resolvedTable === 'image_scans') {
      return {
        ...merged,
        id: merged.id,
        user_id: merged.user_id || merged.userId || '',
        user_email: merged.user_email || merged.userEmail || '',
        filename: merged.filename || '',
        mime_type: merged.mime_type || merged.mimeType || 'image/jpeg',
        file_size: typeof merged.file_size === 'number' ? merged.file_size : (merged.fileSize || 0),
        width: typeof merged.width === 'number' ? merged.width : 0,
        height: typeof merged.height === 'number' ? merged.height : 0,
        sha256: merged.sha256 || '',
        phash: merged.phash || '',
        dhash: merged.dhash || '',
        status: merged.status || 'completed',
        score: typeof merged.score === 'number' ? merged.score : 0,
        ai_score: typeof merged.ai_score === 'number' ? merged.ai_score : (merged.aiScore || 0),
        manipulation_score: typeof merged.manipulation_score === 'number' ? merged.manipulation_score : (merged.manipulationScore || 0),
        authenticity_score: typeof merged.authenticity_score === 'number' ? merged.authenticity_score : (merged.authenticityScore || 0),
        classification: merged.classification || 'Indéterminé',
        confidence: merged.confidence || 'Moyenne',
        explanation: merged.explanation || '',
        created_at: merged.created_at || merged.createdAt || new Date().toISOString(),
        completed_at: merged.completed_at || merged.completedAt || undefined,
        fact_check_id: merged.fact_check_id || merged.factCheckId || undefined,
        metadata: merged.metadata || baseData.metadata || {},
        ai_analysis: merged.ai_analysis || merged.aiAnalysis || baseData.ai_analysis || {},
        forensics: merged.forensics || baseData.forensics || {},
        provenance: merged.provenance || baseData.provenance || {},
        public_url: merged.public_url || merged.publicUrl || undefined,
        image_url: merged.image_url || merged.imageUrl || undefined
      } as unknown as T;
    }
  ...
  async setDoc<T extends { id?: string }>(table: string, id: string, docData: T, options?: { merge?: boolean }): Promise<void> {
    ...
    try {
      ...
      } else if (resolvedTable === 'image_scans') {
        const scan = normalizedData as any;
        payload = {
          ...payload,
          user_id: scan.user_id || scan.userId || '',
          user_email: scan.user_email || scan.userEmail || '',
          filename: scan.filename || '',
          score: typeof scan.score === 'number' ? scan.score : 0,
          classification: scan.classification || '',
          confidence: scan.confidence || '',
          explanation: scan.explanation || '',
          created_at: scan.created_at || scan.createdAt || new Date().toISOString(),
          data: normalizedData
        };
      }
...
export const imageCheckService = {
  async listScans(userId?: string): Promise<ImageScanReport[]> {
    const all = await supabaseDb.list<ImageScanReport>('image_scans');
    if (userId) {
      return all.filter(s => (s.user_id === userId || (s as any).userId === userId));
    }
    return all;
  },
  async getScanById(id: string): Promise<ImageScanReport | null> {
    return supabaseDb.getDoc<ImageScanReport>('image_scans', id);
  },
  async saveScan(scan: ImageScanReport): Promise<void> {
    return supabaseDb.setDoc('image_scans', scan.id, scan, { merge: true });
  },
  async deleteScan(id: string): Promise<void> {
    return supabaseDb.deleteDoc('image_scans', id);
  },
  subscribe(callback: (scans: ImageScanReport[]) => void, userId?: string) {
    return supabaseDb.subscribe<ImageScanReport>('image_scans', (all) => {
      if (userId) {
        callback(all.filter(s => (s.user_id === userId || (s as any).userId === userId)));
      } else {
        callback(all);
      }
    });
