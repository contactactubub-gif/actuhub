...
// ==========================================
// ACTUHUB IMAGE CHECK TYPES & INTERFACES
// ==========================================

export type ImageScanStatus = 'processing' | 'completed' | 'failed';

export type ImageScanClassification = 
...
export type ImageScanConfidence = 'low' | 'medium' | 'high';
...
export interface ImageMetadataInfo {
  ...
  software?: string;
  date_taken?: string;
  gps?: string | { latitude: number; longitude: number };
  orientation?: string;
...
export interface ImageAiAnalysis {
  ...
  human_probability: number; // 0 to 100
...
export interface ImageForensics {
  ela_score: number; // 0 to 100
  compression_score: number; // 0 to 100
  noise_score: number; // 0 to 100
...
export interface ImageScanReport {
  ...
  score: number; // Actuhub Image Score 0 to 100
  ai_score: number;
